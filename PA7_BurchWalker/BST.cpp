...
BST::BST() 
{
	...
	{
		istringstream morseCode(line);

		getline(morseCode, lineData, ' ');
...
void BST::insertHelper(BSTNode* pTree, Data& newData) 
{
	...
	if (newData.letter < pTree->getData().letter)
	{
...
string BST::search(char query) 
{ 
	return searchHelper(this->mpRoot, toupper(query)); 
}
